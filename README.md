...
- 支持自动滚动，并可调节滚动速度。
- 手动阅读时，在章末继续向下滚动或按 `Page Down` 自动进入下一章；自动滚动会在最后一屏留出阅读时间后续章，读完最后一章自动停止。
...
阅读时可按住正文任意位置拖动窗口，无需寻找顶部拖动条；工具栏空白处也能拖动。单击仍显示/收起控制浮层，滚轮保持阅读滚动，窗口边缘用于缩放。不提供正文文字划选或标注；拖动时暂停自动滚动与鼠标移出隐藏，松手后恢复。

## 版本更新与发布

从 0.2.0 开始，首页底部提供“检查更新”：Windows 安装版支持下载并重启安装，macOS 检查后跳转 Release 手动下载。更新由用户触发，阅读时不会主动弹窗。Windows 不再提供便携版。

已配置 GitHub Actions：推送版本标签后自动构建各平台安装包，并生成待确认的 Release 草稿。首次发布、后续升级以及 Actions 操作步骤见 [发布指南](./docs/RELEASING.md)。
